...
func (p *DataOrientedPredictor) getDataParallelMiniBatchDurationNanoSecond(ctx *PredictSessionContext, allocation *objects.JobAllocation) int64 {
	acceleratorType := func() string {
		...
		return ctx.clusterManager.GetAccelerator(acceleratorID).GetAcceleratorMetaInfo().GetBriefType()
	}()
...
func (p *DataOrientedPredictor) getMiniBatchDurationNanoSecond(ctx *PredictSessionContext, jobID string, acceleratorType string) int64 {
	//tmp1 := p.getDLTJobData(jobID).GetAcceleratorType2MiniBatchDuration()
	//tmp2:= tmp1.GetAccType2Duration()
	//return tmp2[acceleratorType]
...
func (p *DataOrientedPredictor) getSingleTaskSpaceSharingMiniBatchDuration(ctx *PredictSessionContext, acceleratorID string, jobIDs []string) map[string]int64 {
	...
	return p.getSpaceSharingMiniBatchDurationNanoSecond(ctx, ctx.clusterManager.GetAccelerators([]string{acceleratorID}), p.getJobs(ctx, jobIDs))
}
...
func (p *DataOrientedPredictor) getDataParallelTasksSpaceSharingMiniBatchDuration(ctx *PredictSessionContext, acceleratorIDs []string, jobIDs []string) map[string]int64 {
	...
	return p.getSpaceSharingMiniBatchDurationNanoSecond(ctx, ctx.clusterManager.GetAccelerators(acceleratorIDs), p.getJobs(ctx, jobIDs))
}
